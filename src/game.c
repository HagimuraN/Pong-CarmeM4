...
int Change_Y_Velocity (unsigned int paddle_coordinate_y);
...
bool Game (unsigned int *ball_coordinate_x, unsigned int *ball_coordinate_y,
		   unsigned int paddle_left_coordinate_x, unsigned int paddle_right_coordinate_y,
		   unsigned int paddle_right_coordinate_x, unsigned int paddle_right_coordinate_y,
		   unsigned char *point_player_r, unsigned char *point_player_l)
{	
	...
	if (gaming)
	{
		if (Move_Ball(&ball_coordinate_x, &ball_coordinate_y))
		{
			...
			Change_Y_Velocity(ball_coordinate_y);
		}
					   ...
					   unsigned int paddle_right_coordinate_x, unsigned int paddle_right_coordinate_y)
{
	if (ball_velocity_x > 0) 	//check collisions for the case when the ball moves twoards right
	{
		if  (((ball_coordinate_x + BALL_WIDTH) >= paddle_right_coordinate_x)     //check if any pixel of ball is next to the pixels of the right paddle 
		  && ((ball_coordinate_y + BALL_HIGHT - 1) >= paddle_right_coordinate_y) //check if the pixel at the bottom of ball is at the same hight like the right paddle.
		  &&  (ball_coordinate_y <= (paddle_right_coordinate_y + PADDLE_HIGHT - 1)))
		{
			ball_velocity_x = -1 * ball_velocity_x;	//reverse the direction
			ball_velocity_y = Change_Y_Velocity(paddle_right_coordinate_y); //calculate new y velocity
		}
	}
	
	if (ball_velocity_x < 0)	//check for collisions for the case when the ball moves twoards left
	{
		if (((ball_coordinate_x <= paddle_left_coordinate_x + BADDLE_WIDTH))	//same as the checking for the right paddle, just for the left baddle.
		 && ((ball_coordinate_y + BALL_HIGHT -1) >= paddle_left_coordinate_y) 
		 && (ball_coordinate_y <= (paddle_left_coordinate_y + PADDLE_HIGHT -1)))
		{
			ball_velocity_x = -1 * ball_velocity_x; //reverse the direciton
			ball_velocity_y = Change_Y_Velocity(paddle_left_coordinate_y);	//calculate new y velocity
		}
	}

	if (((ball_coordinate_y + BALL_HIGHT) == DISP_HIGHT) && (ball_coordinate_y == 0))	//check for bottom and top bouncing.
...
//returns the new ball velocity of y axis
{
	...
	if (BALL_HIGHT > 1)
		bcy_mid = ball_coordinate_y + BALL_HIGHT / 2 - 1;
	else
		bcy_mid = 1;

	if (PADDLE_HIGHT > 1)
		pcy_mid = paddle_coordinate_y + PADDLE_HIGHT / 2 - 1;
	else
		pcy_mid = 1;

	return (BALL_VELOCITY / (PADDLE_HIGHT - (PADDLE_HEIGHT/2))) * (bcy_mid - pcy_mid);
}
